...
def pagerank(G, alpha=0.85, personalization=None, max_iter=100, tol=1.0e-6, nstart=None, weight='weight', dangling=None):
    ...
    # Choose a fixed starting vector if not given
    if nstart is None:
    ...
    if personalization is None:
        # Assign a uniform personalization vector if not given
    ...
    if dangling is None:
        # Use the personalization vector if the dangling vector is not specified
        dangling_weights = p
    else:
    ...
    for _ in range(max_iter):
        ...
        for n in x:
            # This matrix multiplication looks odd because it is doing a left multiply x^T = xlast^T * W
            for nbr in W[n]:
...
# Create a Barabási-Albert graph with 40 nodes and 15 edges to attach from each new node
